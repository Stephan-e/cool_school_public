...
from users.pagination import ResultsSetPagination

from config import settings
from users.models import *
from users.serializers import * 
from users.authentication import AdminAuthentication, UserAuthentication
from users.permissions import *
from users.pagination import *
from users.enums import Role
from users.filters import AdminStudentFilterSet, AdminClientFilterSet
from users.common import get_student_hours, get_client_hours, get_client_hours_penalty
...
def root(request, format=None):
    return Response(
        [
            {'Admin': OrderedDict([
                ('Register', reverse('users:admin-register',
                    request=request,
                    format=format)),
                ('Login', reverse('users:admin-login',
                    request=request,
                    format=format)),
                ('Logout', reverse('users:admin-logout',
                    request=request,
                    format=format)),
                ('Password Change', reverse('users:admin-password-change',
                    request=request,
                    format=format)),
                ('Password Reset', reverse('users:admin-password-reset',
                    request=request,
                    format=format)),
                # ('Club', reverse('users:user-club-view',
                #     request=request,
                #     format=format)),
                ('Booking', reverse('users:admin-booking-view',
                    request=request,
                    format=format)),
                ('Student', reverse('users:admin-students-view',
                    request=request,
                    format=format)),
                ('Client', reverse('users:admin-parent-view',
                    request=request,
                    format=format)),
                ('Teacher', reverse('users:admin-teacher-list',
                    request=request,
                    format=format)),    
                ('Club Session', reverse('users:admin-session-list',
                    request=request,
                    format=format)),
                ('Camp Session', reverse('users:admin-campsession-list',
                    request=request,
                    format=format)),
                ('Trial Session', reverse('users:admin-trialsession-list',
                    request=request,
                    format=format)),
                ('Student Index', reverse('users:admin-students-index',
                    request=request,
                    format=format)),
                ('Client Index', reverse('users:admin-clients-index',
                    request=request,
                    format=format)),
                ('Teacher Index', reverse('users:admin-teacher-index',
                    request=request,
                    format=format)),
            ...
            {'User': OrderedDict([
                ('Register', reverse('users:user-register',
                    request=request,
                    format=format)),
                ('Login', reverse('users:user-login',
                    request=request,
                    format=format)),
                ('Logout', reverse('users:user-logout',
                    request=request,
                    format=format)),
                ('Password Change', reverse('users:user-password-change',
                    request=request,
                    format=format)),
                ('Password Reset', reverse('users:user-password-reset',
                    request=request,
                    format=format)),
                # ('Club', reverse('users:user-club-view',
                #     request=request,
                #     format=format)),
                ('User', reverse('users:user-view',
                    request=request,
                    format=format)),
                ('Booking', reverse('users:user-booking-view',
                    request=request,
                    format=format)),
...
class AdminClientCreateView(ListAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # url = os.environ.get('PWD_SET_URL', 'https://app.usersprojects.com/password/create/')
