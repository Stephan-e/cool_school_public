...
class StudentSerializer(serializers.ModelSerializer):
    ...
    # current_club = serializers.CharField(source='current_club.club.name',read_only=True)
    status = serializers.CharField(required=False)
    location = serializers.CharField(required=False)
...
class UpdateStudentSerializer(serializers.ModelSerializer):
    ...
    # current_club = serializers.CharField(source='current_club.club.name',read_only=True)
    status = serializers.CharField(required=False)
    # location = serializers.ChoiceField(required=False,choices=Locations)
...
class StudentShorterSerializer(serializers.ModelSerializer):
    ...
    def get_age(self, instance):
        today=datetime.datetime.today()
        return today.year - instance.birth_date.year - ((today.month, today.day) < (instance.birth_date.month, instance.birth_date.day))
 #   return datetime.datetime.now().year - instance.birth_date.year
...
class CreateStudentSerializer(StudentSerializer):
    ...
    language = serializers.CharField()
    medical_condition = serializers.CharField(required=False)
    # enrollment = serializers.BooleanField(required=False)
    student_id = serializers.CharField(required=False)
    # student_note = serializers.CharField(required=False)
...
class CreateStudentSessionSerializer(serializers.ModelSerializer):
    location = serializers.CharField()
    # day = serializers.ChoiceField(choices=Days.choices())
    day = serializers.CharField()
    time = serializers.CharField()
    club = serializers.CharField()
...
class UpdateStudentSessionSerializer(serializers.ModelSerializer):
    ...
    #     try:
    #         student = Student.objects.get(
    #             identifier=validated_data['session'],
    #         )
    #     except Student.DoesNotExist:
    #         raise exceptions.NotFound()
...
class CreateStudentCampSessionSerializer(serializers.ModelSerializer):
    # location = serializers.ChoiceField(choices=Locations.choices())
    location = serializers.CharField()
    club = serializers.CharField()
    teacher = serializers.CharField(required=False, allow_blank=True)
    duration = serializers.DecimalField(max_digits=5, decimal_places=2)
    ...
    status = serializers.ChoiceField(
                ...
                allow_blank=True)    # end_date = serializers.DateField(required=False)
...
class UpdateStudentBookingSerializer(StudentBookingSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if get_client_hours(client) < 8:
            send_mail(
            "Skyrock Hours Depleted " + str(client.first_name) + " " + str(client.last_name),
            ...
            ["admissions@skyrockprojects.com","ethan@skyrockprojects.com","stephan@skyrockprojects.com"],
...
class UserUpdateSerializer(DateSerializer):
    ...
    #         try:
    #             instance.client = Client.objects.get(identifier=client['identifier'])
    #         except Client.DoesNotExist:
    #             raise exceptions.NotFound()
        
    #     if validated_data.get('staff'):
    #         try:
    #             instance.staff = Staff.objects.get(identifier=staff['identifier'])
    #         except Staff.DoesNotExist:
    #             raise exceptions.NotFound()

    #     if validated_data.get('teacher'):
    #         try:
    #             instance.teacher = Teacher.objects.get(identifier=teacher['identifier'])
    #         except Teacher.DoesNotExist:
    #             raise exceptions.NotFound()
    ...
    #         try:
    #             user.client = Client.objects.get(identifier=client)
    #         except Client.DoesNotExist:
    #             raise exceptions.NotFound()
        
    #     if request.data.get('staff'):
    #         try:
    #             user.staff = Staff.objects.get(identifier=staff)
    #         except Staff.DoesNotExist:
    #             raise exceptions.NotFound()

    #     if request.data.get('teacher'):
    #         try:
    #             user.teacher = Teacher.objects.get(identifier=teacher)
    #         except Teacher.DoesNotExist:
    #             raise exceptions.NotFound()
